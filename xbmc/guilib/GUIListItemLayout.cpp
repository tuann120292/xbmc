...
void CGUIListItemLayout::LoadControl(TiXmlElement *child, CGUIControlGroup *group, GUIResourceProviderPtr provider)
{
  ...
  if (control)
  {
    ...
    if (control->IsGroup())
    {
      ...
      while (grandChild)
      {
        LoadControl(grandChild, (CGUIControlGroup *)control, provider);
...
void CGUIListItemLayout::LoadLayout(TiXmlElement *layout, int context, bool focused, GUIResourceProviderPtr provider)
{
  m_focused = focused;
