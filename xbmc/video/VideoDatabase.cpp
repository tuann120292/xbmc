  ...
  try
  {
    ...
    if (g_settings.GetMasterProfile().getLockMode() != LOCK_MODE_EVERYONE && !g_passwordManager.bMasterUser)
    {
      ...
      for (it=mapSeasons.begin();it != mapSeasons.end();++it)
      {
        ...
        if (it->second.numEpisodes > 0)
          pItem->SetProperty("percentwatched", static_cast<int>(static_cast<double>(it->second.numWatched) / static_cast<double>(it->second.numEpisodes)*100));
        else
    ...
    else
    {
      while (!m_pDS->eof())
      {
        ...
        if (totalEpisodes > 0)
          pItem->SetProperty("percentwatched", static_cast<int>(static_cast<double>(watchedEpisodes) / static_cast<double>(totalEpisodes)*100));
        else
          pItem->SetProperty("percentwatched", 0);
        if (iSeason == 0) pItem->SetProperty("isspecial", true);
    ...
    for (DatabaseResults::const_iterator it = results.begin(); it != results.end(); it++)
    {
          ...
          (!g_advancedSettings.m_bVideoLibraryHideEmptySeries || movie.m_iEpisode > 0))
      {
        ...
        if (movie.m_iEpisode > 0)
          pItem->SetProperty("percentwatched", static_cast<int>(static_cast<double>(movie.m_playCount) / static_cast<double>(movie.m_iEpisode)*100));
        else
          pItem->SetProperty("percentwatched", 0);
